...
/// The AI-generated declaration keyword (07 §3.1 AI 生成内容标识).
pub const AI_GENERATED_KEYWORD: &str = "AI-Generated";
/// The declaration text written into every generated PNG.
pub const AI_GENERATED_DECLARATION: &str =
    "This image was generated by AI using MoFA Studio. Do not use to mislead.";
...
/// Tag a PNG as AI-generated (07 §3.1). Idempotent — an existing tag is
/// replaced, not duplicated.
pub fn tag_ai_generated(png: &[u8]) -> Result<Vec<u8>, String> {
    let chunks = chunks(png).ok_or("not a PNG")?;
    let payload = format!("{AI_GENERATED_KEYWORD}\0{AI_GENERATED_DECLARATION}");
    let mut out = PNG_SIGNATURE.to_vec();
    for (kind, data) in chunks {
        if kind == b"IEND" {
            // Insert the tag right before the terminal chunk, then IEND itself.
            out.extend_from_slice(&chunk(b"tEXt", payload.as_bytes()));
            out.extend_from_slice(&chunk(kind, data));
        } else if kind == b"tEXt" {
            let keyword_len = data.iter().position(|&b| b == 0).unwrap_or(0);
            if &data[..keyword_len] == AI_GENERATED_KEYWORD.as_bytes() {
                continue; // replace, not duplicate
            }
            out.extend_from_slice(&chunk(kind, data));
        } else {
            out.extend_from_slice(&chunk(kind, data));
...
/// Whether a PNG carries the AI-generated declaration.
pub fn is_ai_generated(png: &[u8]) -> bool {
    let Some(chunks) = chunks(png) else {
        return false;
    };
    for (kind, data) in chunks {
        if kind != b"tEXt" {
        ...
        if &data[..split] == AI_GENERATED_KEYWORD.as_bytes() {
            return true;
        }
    }
    false
...
mod tests {
    ...
    #[test]
    fn ai_generated_tag_round_trip_and_idempotent() {
        let png = minimal_png();
        // Not tagged initially.
        assert!(!is_ai_generated(&png));

        // Tag it.
        let tagged = tag_ai_generated(&png).unwrap();
        assert!(is_ai_generated(&tagged));

        // Tagging again replaces, not duplicates.
        let tagged_twice = tag_ai_generated(&tagged).unwrap();
        assert!(is_ai_generated(&tagged_twice));
        let tag_count = chunks(&tagged_twice)
            .unwrap()
            .iter()
            .filter(|(k, d)| {
                *k == b"tEXt"
                    && d.iter()
                        .position(|&b| b == 0)
                        .map(|len| &d[..len] == AI_GENERATED_KEYWORD.as_bytes())
                        .unwrap_or(false)
            })
            .count();
        assert_eq!(tag_count, 1, "exactly one AI-Generated tag");

        // Non-PNG rejected.
        assert!(tag_ai_generated(b"JFIF").is_err());
    }

    #[test]
    fn ai_tag_coexists_with_workflow_tag() {
        let png = minimal_png();
        let workflow = embed_workflow(&png, r#"{"nodes":[]}"#).unwrap();
        let both = tag_ai_generated(&workflow).unwrap();
        assert!(is_ai_generated(&both));
        assert!(extract_workflow(&both).is_some());
    }
