...
import { skillHubClient, type SkillSearchParams, type SkillSummary, type SkillDetail } from '@/services/skill-hub-client'
...
export function useSkill(skillId: number | null) {
  ...
  useEffect(() => {
    ...
    skillHubClient.getSkill(skillId)
      .then(setData)
      .catch((err) => {
        setError(err)
...
export function useSkillByNamespace(namespace: string, slug: string) {
  ...
  useEffect(() => {
    ...
    skillHubClient.getSkillByNamespace(namespace, slug)
      .then(setData)
      .catch((err) => {
        setError(err)
        console.error('Failed to fetch skill:', err)
...
export function useStar(skillId: number | null, enabled = true) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    skillHubClient.checkStarred(skillId)
      .then(setData)
 ...
 */
export function useSkillVersions(skillId: number | null, page = 1, pageSize = 10) {
  const [data, setData] = useState<{ versions: any[]; total: number }>({ versions: [], total: 0 })
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<Error | null>(null)
  ...
  useEffect(() => {
    ...
    skillHubClient.listVersions(skillId, page, pageSize)
      .then(setData)
      .catch((err) => {
        setError(err)
...
export function useNamespaces(tenantId: string) {
  ...
  useEffect(() => {
    ...
    skillHubClient.listNamespaces(tenantId)
      .then(setData)
      .catch((err) => {
        setError(err)
...
export function useLabels(tenantId: string, type?: string) {
  ...
  useEffect(() => {
    ...
    skillHubClient.listLabels(tenantId, type)
      .then(setData)
      .catch((err) => {
        setError(err)
