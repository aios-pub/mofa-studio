...
import { Input, InputNumber, Select, Switch, Collapse, Tag, Checkbox, Alert } from 'antd';
import {
  KeyOutlined,
  LinkOutlined,
  SettingOutlined,
      ...
      <div className="bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)] p-4">
        <h4 className="text-sm font-medium text-[var(--color-text-primary)] mb-3 flex items-center gap-2">
          ...
          <span className="text-xs text-[var(--color-text-tertiary)] font-normal ml-auto">
            {formData.selectedModels?.length || 0} / {config.defaultModels.length} 个已选
          </span>
        </h4>
        ...
        <div className="mb-3">
          <button
            type="button"
            onClick={() => {
              const allSelected = formData.selectedModels?.length === config.defaultModels.length;
              onChange({
                selectedModels: allSelected
                  ? []
                  : config.defaultModels.map(m => m.id),
              });
            }}
            className="text-xs text-[var(--color-primary)] hover:underline"
          >
            {formData.selectedModels?.length === config.defaultModels.length ? '取消全选' : '全选'}
          </button>
        </div>
...
const ModelItem: React.FC<ModelItemProps> = ({ model, checked, onChange }) => {
