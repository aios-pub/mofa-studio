...
export default function OctosGatewaySettingsTab({ config, onChange }: Props) {  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-4">
      <Alert
        ...
        showIcon
        title={t("网关设置")}
        description={t("配置 Agent 行为参数：对话记忆、工具调用限制、系统提示词等。")}
        className="text-xs"
      />

      <div>
        <Text type="secondary" className="block mb-1">{t("最大历史记录")}</Text>
        <InputNumber
          ...
          style={{ width: 200 }}
        />
        <div><Text type="secondary" className="text-xs">{t("对话历史中保留的最大消息数")}</Text></div>
      </div>

      <div>
        <Text type="secondary" className="block mb-1">{t("最大迭代次数")}</Text>
        <InputNumber
          ...
          style={{ width: 200 }}
        />
        <div><Text type="secondary" className="text-xs">{t("每次 Agent 调用的最大工具调用迭代次数")}</Text></div>
      </div>

      <div>
        <Text type="secondary" className="block mb-1">{t("最大并发会话")}</Text>
        <InputNumber
          ...
          style={{ width: 200 }}
        />
        <div><Text type="secondary" className="text-xs">{t("最大并发聊天会话数（默认无限制）")}</Text></div>
      </div>

      <div>
        <Text type="secondary" className="block mb-1">{t("浏览器超时（秒）")}</Text>
        <InputNumber
          ...
          style={{ width: 200 }}
        />
        <div><Text type="secondary" className="text-xs">{t("无头浏览器工具操作的超时时间")}</Text></div>
      </div>

      <div>
        <Text type="secondary" className="block mb-1">{t("最大输出 Tokens")}</Text>
        <InputNumber
          ...
          style={{ width: 200 }}
        />
        <div><Text type="secondary" className="text-xs">{t("每次 LLM 调用的默认最大输出 tokens")}</Text></div>
      </div>

      <div>
        <Text type="secondary" className="block mb-1">{t("系统提示词")}</Text>
        <Input.TextArea
          ...
          rows={4}
        />
        <div><Text type="secondary" className="text-xs">{t("此网关实例的自定义系统提示词")}</Text></div>
      </div>
      ...
      <Collapse
        items={[
          {
            key: "hooks",
            label: <Text strong>{t("Hooks 配置")}</Text>,
            children: (
              <div className="space-y-4">
                <Alert
                  type="info"
                  showIcon
                  title={t("事件钩子")}
                  description={t("在指定事件发生时执行自定义命令，可用于通知、日志记录等。")}
                  className="text-xs"
                />

                {/* Add hook form */}
                <Form form={hooksForm} layout="inline" className="mb-4">
                  <Form.Item name="event" label={t("事件")} initialValue="tool_call">
                    <Select
                      style={{ width: 150 }}
                      options={[
                        { label: t("工具调用"), value: "tool_call" },
                        { label: t("消息发送"), value: "message_send" },
                        { label: t("对话结束"), value: "conversation_end" },
                        { label: t("错误发生"), value: "error" },
                      ]}
                    />
                  </Form.Item>
                  <Form.Item name="command" label={t("命令")}>
                    <Input placeholder="/path/to/script.sh" />
                  </Form.Item>
                  <Form.Item name="timeout_ms" label={t("超时(ms)")}>
                    <InputNumber placeholder="30000" style={{ width: 120 }} />
                  </Form.Item>
                  <Form.Item name="tool_filter" label={t("工具过滤")}>
                    <Input placeholder={t("web_search,code_exec (逗号分隔)")} />
                  </Form.Item>
                  <Form.Item>
            ...
            children: (
              <div className="space-y-4">
                <Alert
                  ...
                  showIcon
                  title={t("沙箱模式")}
                  description={t("配置代码执行和安全隔离环境。Docker 模式提供最强的隔离性。")}
                  className="text-xs"
                />

                <div>
                  <Text type="secondary" className="block mb-2">{t("启用沙箱")}</Text>
                  <Switch
                    checked={config.sandbox?.enabled || false}
                    onChange={(v) => updateSandbox({ ...config.sandbox, enabled: v } as SandboxConfig)}
                  />
                  <div className="mt-1"><Text type="secondary" className="text-xs">{t("启用后，代码执行将在隔离环境中运行")}</Text></div>
                </div>

                {config.sandbox?.enabled && (
                  <>
                    <div>
                      <Text type="secondary" className="block mb-2">{t("沙箱模式")}</Text>
                      <Select
                        value={config.sandbox.mode || "auto"}
                        onChange={(v) => updateSandbox({ ...config.sandbox, mode: v } as SandboxConfig)}
                        style={{ width: 300 }}
                        options={[
                          { label: t("自动（推荐）"), value: "auto" },
                          { label: t("Docker（最强隔离）"), value: "docker" },
                          { label: t("macOS 沙箱"), value: "macos" },
                    ...
                    <div>
                      <Text type="secondary" className="block mb-2">{t("允许网络访问")}</Text>
                      <Switch
                        checked={config.sandbox.allow_network || false}
                        onChange={(v) => updateSandbox({ ...config.sandbox, allow_network: v } as SandboxConfig)}
                      />
                      <div className="mt-1"><Text type="secondary" className="text-xs">{t("允许沙箱内的代码访问网络")}</Text></div>
                    </div>

                    {config.sandbox.mode === "docker" && (
                      <div className="p-3 bg-blue-50 rounded space-y-3">
                        <Text strong className="text-sm">{t("Docker 配置")}</Text>
                        <div>
                          <Text type="secondary" className="block mb-1 text-xs">{t("镜像")}</Text>
                          <Input
                        ...
                        <div>
                          <Text type="secondary" className="block mb-1 text-xs">{t("CPU 限制")}</Text>
                          <Input
                        ...
                        <div>
                          <Text type="secondary" className="block mb-1 text-xs">{t("内存限制")}</Text>
                          <Input
