...
export default function ExecutionsTab() {
  ...
  const filteredExecutions = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (e) => e.taskName.toLowerCase().includes(q)
  ...
  const columns: ColumnsType<TaskExecution> = [
    ...
    },
    {
      ...
      key: 'status',
      width: 90,
      filters: Object.entries(execStatusLabel).map(([k, v]) => ({ text: v, value: k })),
    ...
    },
    {
      ...
      render: (_: unknown, record: TaskExecution) => (
        <span className={record.error ? 'text-red-500' : 'text-[var(--color-text-primary)]'}>
      ...
      <div className="flex items-center gap-3 p-3 border-b border-[var(--color-border)]">
        ...
        <div className="flex gap-1.5">
          {[
            ...
            { key: 'success' as const, label: '成功', count: counts.success, color: 'text-green-500' },
            { key: 'failure' as const, label: '失败', count: counts.failure, color: 'text-red-500' },
            { key: 'running' as const, label: '运行中', count: counts.running, color: 'text-blue-500' },
