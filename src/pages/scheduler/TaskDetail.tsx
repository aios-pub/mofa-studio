...
const getStatusIcon = (status: ExecutionStatus | undefined) => {
  switch (status) {
    ...
    case 'failure': return <CloseCircleOutlined className="text-red-500" />;
    case 'running': return <LoadingOutlined className="text-blue-500" />;
...
export default function TaskDetail({
  ...
  return (
    <div className="lg:grid lg:grid-cols-5 flex-1 overflow-y-auto">
      {/* 左侧：参数 + 上下文 */}
      <div className="lg:col-span-2 flex flex-col overflow-y-auto border-r border-[var(--color-border)]">
        {/* 参数 JSON */}
        <div className="bg-[var(--color-bg-secondary)]">
          <h3 className="p-3 text-sm font-medium text-[var(--color-text-primary)] border-b border-[var(--color-border)]">
            参数 (Arguments)
          </h3>
        ...
        <div className="bg-[var(--color-bg-secondary)] flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-0">
            {contextItems.map((item) => (
              <div key={item.key} className="flex justify-between items-center py-2 border-b border-[var(--color-border)] last:border-b-0">
                <span className="text-sm text-[var(--color-text-tertiary)]">{item.key}</span>
                {item.tag ? (
                  <Tag color={task.status === 'enabled' ? 'green' : 'default'}>{item.value}</Tag>
                ) : (
                  <span className={`text-sm text-[var(--color-text-primary)] truncate max-w-[60%] text-right ${item.mono ? 'font-mono' : ''} ${item.highlight === 'green' ? 'text-green-500' : item.highlight === 'orange' ? 'text-orange-500' : item.highlight === 'red' ? 'text-red-500' : ''}`}>
                    {item.value}
                  </span>
                )}
              </div>
      ...
      <div className="lg:col-span-3 flex flex-col overflow-y-auto">
        {/* 头部操作栏 */}
        <div className="p-3 border-b border-[var(--color-border)] flex items-center justify-between bg-[var(--color-bg-secondary)]">
          ...
          <div className="flex gap-1.5">
            <Tooltip title="执行">
              <Button size="small" type="primary" icon={<CaretRightOutlined />} onClick={() => onExecute(task.id)} />
            </Tooltip>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="divide-y divide-[var(--color-border)]">
            {taskExecutions.length === 0 ? (
              <p className="text-xs text-[var(--color-text-tertiary)] p-3">暂无执行记录</p>
            ) : (
              taskExecutions.map((exec) => (
                <div key={exec.id} className="flex items-center gap-3 px-3 py-2 hover:bg-[var(--color-bg-tertiary)] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <Tag color={execStatusColorMap[exec.status]} className="ml-2 flex-shrink-0">
