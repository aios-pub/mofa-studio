...
export default function TasksTab({ initialFilterType, onFilterTypeConsumed }: {
  ...
  const filteredTasks = useMemo(() => {
    switch (statusTab) {
      case 'enabled': return tasks.filter((t) => t.status === 'enabled');
      case 'disabled': return tasks.filter((t) => t.status === 'disabled');
      case 'running': return tasks.filter((t) => t.lastRunStatus === 'running');
  ...
  const handleExecute = async (id: string) => {
    try { await scheduledTaskApi.executeTask(id); message.success('任务已开始执行'); loadData(); }
    catch { message.error('执行失败'); }
  };

  const handleToggle = async (id: string) => {
    try { await scheduledTaskApi.toggleTask(id); message.success('状态已更新'); loadData(); }
  ...
  const columns: ColumnsType<ScheduledTask> = [
    ...
    },
    {
      ...
      render: (cron: string) => (
        <div>
          <code className="text-xs font-mono text-[var(--color-text-tertiary)]">{cron}</code>
          <div className="text-xs text-[var(--color-text-tertiary)]">{parseCronToText(cron)}</div>
        </div>
      ),
    },
    {
      ...
      render: (_: unknown, record: ScheduledTask) => {
        const rate = record.successCount + record.failureCount > 0
          ? (record.successCount / (record.successCount + record.failureCount)) * 100 : null;
        return (
          <div className="flex items-center gap-2 text-xs">
            <span className="flex items-center gap-0.5 text-green-500">
              <CheckCircleOutlined />{record.successCount}
            </span>
            <span className="flex items-center gap-0.5 text-red-500">
              <CloseCircleOutlined />{record.failureCount}
            </span>
            {rate !== null && (
              <span className={rate >= 90 ? 'text-green-500' : rate >= 70 ? 'text-orange-500' : 'text-red-500'}>
                {rate.toFixed(0)}%
              </span>
            )}
          </div>
      ...
      render: (v: Date | undefined, record: ScheduledTask) => (
        <div className="text-xs">
          ...
          {record.lastRunStatus && (
            <Tag color={record.lastRunStatus === 'success' ? 'green' : record.lastRunStatus === 'failure' ? 'red' : 'blue'} className="text-xs mt-0.5">
              {record.lastRunStatus === 'success' ? '成功' : record.lastRunStatus === 'failure' ? '失败' : '运行中'}
            </Tag>
          )}
        </div>
      ),
    },
    {
      ...
      key: 'nextRun',
      width: 150,
      render: (v: Date | undefined) => <span className="text-xs">{formatDate(v)}</span>,
    },
    {
      title: '操作',
      key: 'actions',
      ...
      render: (_: unknown, record: ScheduledTask) => (
        <div className="flex gap-1">
          <Button size="small" type="primary" icon={<CaretRightOutlined />} onClick={() => handleExecute(record.id)} />
          <Button
            size="small"
            icon={record.status === 'enabled' ? <PauseOutlined /> : <PlayCircleOutlined />}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 p-3 border-b border-[var(--color-border)]">
        ...
        <Button type="primary" icon={<PlusOutlined />} size="small" onClick={() => setShowModal(true)}>
          创建任务
        </Button>
      </div>
      ...
      <div className="flex-1 overflow-hidden">
        <Table
          ...
          onRow={(record) => ({
            ...
            className: selectedTask?.id === record.id
      ...
      {/* 详情面板 */}
      {selectedTask && (
        <div className="border-t border-[var(--color-border)] h-72 flex-shrink-0 bg-[var(--color-bg-secondary)]">
          <TaskDetail
            task={selectedTask}
