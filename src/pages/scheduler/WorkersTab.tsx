...
export default function WorkersTab() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setHandlers(types.map((t: TaskTypeDescriptor) => ({
        taskType: t.taskType,
        label: t.label,
  ...
  const handlerColumns: ColumnsType<HandlerInfo> = [
    ...
    },
    {
      ...
      dataIndex: 'label',
      key: 'label',
      render: (label: string) => <span className="text-[var(--color-text-primary)]">{label}</span>,
    },
    {
      title: '描述',
      dataIndex: 'description',
      key: 'description',
      render: (desc: string) => <span className="text-xs text-[var(--color-text-tertiary)]">{desc}</span>,
    },
    {
      title: '状态',
      key: 'status',
  ...
  return (
    <div className="p-4 space-y-6 overflow-y-auto h-full">
      ...
      <section>
        ...
        <p className="text-sm text-[var(--color-text-tertiary)] mb-4">Cron 调度工作进程</p>
        <div className="bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)] p-4 flex items-center gap-6">
          <div className="flex items-center gap-2">
            <Badge status="success" />
            <span className="font-medium text-[var(--color-text-primary)]">scheduled-task-worker</span>
          </div>
          <div className="text-xs text-[var(--color-text-tertiary)]">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[var(--color-bg-tertiary)]">
              <ApiOutlined />CronStream (apalis-cron)
            </span>
          </div>
          <div className="text-xs text-[var(--color-text-tertiary)]">
            并发: 1
      ...
      <section>
        ...
        <p className="text-sm text-[var(--color-text-tertiary)] mb-4">
          共 {handlers.length} 个处理函数注册 — 数据来自后端 <code className="text-xs">/api/task/types</code>
        </p>
        <Table
          dataSource={handlers}
