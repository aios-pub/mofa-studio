...
export default function ConversationPage() {
  ...
  const [selectedAgentForNew, setSelectedAgentForNew] = useState<string | undefined>();
      ...
      try {
        ...
        setSelectedConversation((prev) => {
          ...
          return {
            ...
            totalTokens: newMessages.reduce((sum, m) => sum + (m.tokens?.input || 0) + (m.tokens?.output || 0), 0),
            updatedAt: new Date(),
          };
        });
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg focus:outline-none focus:border-[var(--color-primary)]"
            >
              {agents.map((agent) => (
          ...
          <div>
            <label className="block text-sm font-medium mb-1">对话标题（可选）</label>
            <Input
              placeholder="输入对话标题..."
