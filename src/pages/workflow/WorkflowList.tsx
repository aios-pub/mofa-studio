  ...
  const handleCreateWorkflow = async () => {
    try {
      const newWorkflow = await workflowApi.create({
        ...
        edges: [{ id: 'edge-1', sourceNodeId: 'node-start', targetNodeId: 'node-end' }],
      });
    ...
    <div className="h-full overflow-y-auto bg-[var(--color-bg-base)]">
      ...
      <div className="p-6 border-b border-[var(--color-border)]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">工作流</h1>
...
function WorkflowCard({
  ...
  return (
    <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg p-4 hover:border-[var(--color-primary)]/50 transition-colors group">
      {/* 头部 */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-medium text-[var(--color-text-primary)]">{workflow.name}</h3>
            <Tag color={statusConfig[workflow.status].color} className="text-xs mt-1">
