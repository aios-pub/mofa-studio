...
export default memo(({ data, selected }: NodeProps) => {
  const typeInfo = nodeTypeConfig['parallel' as NodeType];
  ...
  return (
    <div
      className={`
        ...
        ${selected ? 'border-[var(--color-primary)]' : 'border-yellow-500'}
        shadow-sm
      `}
    >
      <Handle
      ...
      <div className="space-y-1">
        ...
        {branches.length === 0 && (
          <div className="text-xs text-[var(--color-text-tertiary)] text-center py-1">
            点击配置分支
          </div>
        )}
      </div>
