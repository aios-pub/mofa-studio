...
export const tracingApi = {
    ...
    getTraces: async (filter?: TracingFilter): Promise<{ data: Trace[] }> => {
        ...
        if (filter) {
            if (filter.trace_id) {
                filtered = filtered.filter((t) => t.trace_id.includes(filter.trace_id!));
            }
            if (filter.service_name) {
                filtered = filtered.filter((t) => t.service_name.includes(filter.service_name!));
            }
            if (filter.operation_name) {
                filtered = filtered.filter((t) => t.operation_name.includes(filter.operation_name!));
            }
            ...
            if (filter.min_duration != null) {
                filtered = filtered.filter((t) => t.total_duration >= filter.min_duration!);
            }
            if (filter.max_duration != null) {
                filtered = filtered.filter((t) => t.total_duration <= filter.max_duration!);
            }
        }
        return {
            data: filtered,
    ...
    getTracingStats: async (): Promise<TracingStats> => {
        ...
        const durations = MOCK_TRACES.map((t) => t.total_duration).sort((a, b) => a - b);
        const traces_by_service: Record<string, number> = {};
        const traces_by_operation: Record<string, number> = {};
        MOCK_TRACES.forEach((t) => {
            traces_by_service[t.service_name] = (traces_by_service[t.service_name] || 0) + 1;
            traces_by_operation[t.operation_name] = (traces_by_operation[t.operation_name] || 0) + 1;
        });
        return {
            total_traces,
            error_traces,
     ...
     * 导出追踪数据
     */
    exportTraces: async (filter?: TracingFilter, format: 'json' | 'csv' = 'json'): Promise<Blob> => {
        await delay(300);
        const { data } = await tracingApi.getTraces(filter);

        if (format === 'csv') {
            // 生成 CSV
            const headers = ['trace_id', 'service_name', 'operation_name', 'start_time', 'total_duration', 'span_count', 'status'];
            const rows = data.map(t =>
                [t.trace_id, t.service_name, t.operation_name, t.start_time, t.total_duration, t.span_count, t.status].join(',')
            );
    ...
    exportTrace: async (traceId: string, format: 'json' | 'csv' = 'json'): Promise<Blob> => {
        ...
        if (format === 'csv') {
            const headers = ['span_id', 'name', 'start_time', 'duration', 'status', 'parent_span_id'];
            const rows = (trace.spans || []).map(s =>
                [s.span_id, s.name, s.start_time, s.duration, s.status, s.parent_span_id || ''].join(',')
            );
            const csv = [headers.join(','), ...rows].join('\n');
            return new Blob([csv], { type: 'text/csv' });
        }

        return new Blob([JSON.stringify(trace, null, 2)], { type: 'application/json' });
    },
};
