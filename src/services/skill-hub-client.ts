...
const API_BASE_URL =
  import.meta.env.VITE_SKILL_HUB_API_URL || "http://localhost:8080";
...
export const skillHubClient = {
  ...
  async downloadSkill(skillId: number, version: string) {
    window.open(
      `${API_BASE_URL}/api/skills/${skillId}/download/${version}`,
      "_blank",
    );
  },
  ...
  async checkStarred(skillId: number) {
    return apiRequest<{ starred: boolean }>(
      `/api/skills/${skillId}/star`,
    ).catch(() => ({ starred: false }));
  },
  ...
  async getSkillRating(skillId: number) {
    return apiRequest<{ rating: number; count: number }>(
      `/api/skills/${skillId}/rating`,
